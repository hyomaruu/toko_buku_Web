'use client'
import { Book, deleteBook, getBooks } from "@toko-buku/controllers/book";
...
export default function Page() {
  ...
  const [data, setData] = useState<Book[] | undefined | null>();
  const [isLoading, setLoading] = useState<boolean>(true);
  const [openModal, setOpenModal] = useState(false);

  const [seletedBook, setSeletedBook] = useState<Book | null>();

  const handleDeleteBook = (user: string) => {
    setLoading(true)
    deleteBook(user).then((response: Response<Book>) => {
      if(response?.data){
       console.log('Succesfully delete book')
      }
    }).finally(() => {
        setOpenModal(false)
        handleFetchData()
  ...
  const handleFetchData = (params?: any) => {
      setLoading(true)
      getBooks(params).then((response: ResponseArray<Book>) => {
      ...
      <div className="my-10" >
        <div className="flex items-center justify-between mb-8">
            ...
            <Link href='/books/add'>
              <Button 
              disabled={isLoading}
              type="button" className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">Tambah Buku</Button>
            </Link>
        </div>

        <div className="mb-8">
          <SearchbarComponent 
          placeholder="Cari bedasarkan nama buku, pengarang, penerbit..."
        ...
        <div className={"relative overflow-x-auto"}>
            <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                ...
                <tbody>
                    {!isLoading && data?.map((book: Book) => {
                      return (
                        <tr key={book.Id} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                        <th scope="row" className="px-6 py-4 font-medium whitespace-nowrap">
                          <div className="flex items-center space-x-3">
                            <img src={book.Image as string} className="bg-cover rounded-lg w-32 h-16" alt="" />
                            <div>
                              <a href={`/books/${book.Id}`} >
                                <div className="hover:text-blue-400 font-semibold text-gray-900 dark:text-white mb-1">{book.Name}</div>
                              </a>
                              <div className="text-sm text-gray-200 dark:text-gray-700">{book.Pengarang}</div>
                            </div>
                          </div>
                        </th>
                        ...
                        <td className="px-6 py-4">
                            {handleFormatCurrency(book.Harga as number)}
                        </td>
                        <td className="px-6 py-4">
                            {book.Stok}
                        </td>
        ...
        <Modal dismissible show={openModal} onClose={() => setOpenModal(false)}>
          <Modal.Header>Hapus Buku</Modal.Header>
          <Modal.Body>
            <div className="space-y-6">
              <p className="text-base leading-relaxed text-gray-500 dark:text-gray-400">
                Dengan ini anda dapat menghapus buku <span className="font-semibold text-gray-100 dark:text-gray-200">{seletedBook?.Name}</span> dengan pengarang <span className="text-gray-100 dark:text-gray-200 font-semibold">{seletedBook?.Pengarang}</span> yang sudah terdaftar didalam aplikasi.
              </p>
              <p className="text-base leading-relaxed text-gray-500 dark:text-gray-400">
          ...
          <Modal.Footer>
            <Button disabled={isLoading} color="red" onClick={() => {
              handleDeleteBook(seletedBook?.Id as unknown as string)
