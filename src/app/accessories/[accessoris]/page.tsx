...
import { Accessoris, getAccessoris } from "@toko-buku/libs/controllers/accessoris";
...
export default function AccessoriesPage(
  ...
  const [data, setData] = useState<Accessoris | null>()
  const [stock, setStock] = useState<number>(490)


  const handleFetchData = (accessorisData: string) => {
      setFetchLoading(true)
      getAccessoris(accessorisData).then((response: Response<Accessoris>) => {
  ...
  useEffect(() => {
    if (!initialized.current) {
      handleFetchData(params?.accessoris)
    ...
    <>
      <div className="my-10">
        <div className="flex items-center justify-between mb-8">
              <h1 className="font-semibold text-xl ">Detail Aksesoris</h1>
        </div>
